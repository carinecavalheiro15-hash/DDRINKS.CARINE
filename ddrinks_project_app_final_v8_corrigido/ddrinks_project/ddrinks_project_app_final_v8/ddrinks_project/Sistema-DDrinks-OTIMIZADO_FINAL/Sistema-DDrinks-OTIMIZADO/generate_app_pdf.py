...
final_output_pdf = os.path.join(base_path, "orcamento_app_otimizado_v10_com_design.pdf")
...
try:
    ...
    print(f"✓ PDF do orçamento gerado: {output_pdf}")
    
    # Mescla com Design PDF se disponível
    if HAS_PYPDF2 and os.path.exists(design_pdf):
        try:
            merger = PdfMerger()
            merger.append(output_pdf)
            merger.append(design_pdf)
            merger.write(final_output_pdf)
            merger.close()
            print(f"✓ Design PDF mesclado com sucesso!")
            print(f"✓ PDF completo (com design) salvo em: {final_output_pdf}")
            
            # Usar o PDF mesclado como saída final
            if os.path.exists(final_output_pdf):
                os.remove(output_pdf)  # Remove o temporário
                os.rename(final_output_pdf, output_pdf)  # Renomeia para o nome final
                print(f"✓ Arquivo final: {output_pdf}")
        except Exception as merge_error:
            print(f"⚠️  Aviso: Não foi possível mesclar com Design PDF: {merge_error}")
            print(f"PDF do orçamento foi gerado sem o design.")
    elif not os.path.exists(design_pdf):
        print(f"⚠️  Aviso: Design PDF não encontrado em {design_pdf}")
        print(f"Gerando apenas o PDF do orçamento.")
    else:
        print(f"⚠️  PyPDF2 não instalado. Para mesclar PDFs com design, instale: pip install PyPDF2")
        print(f"Por enquanto, apenas o PDF do orçamento foi gerado.")
